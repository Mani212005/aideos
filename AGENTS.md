...
- The connector runs each task headless in an empty scratch folder, confined to the aideos MCP tools served statelessly at `POST /api/mcp` (`createMcpServer({ remote: true })`, which drops the production-run tools; design file access is limited by `backend/mcp/designTools.ts`). Claude runs with `--tools ""`; Antigravity with `--sandbox` (its MCP server is registered globally with `agy mcp add`); Codex with `--sandbox read-only`; OpenCode with edit/bash/webfetch denied. `--model provider/model` overrides the run model for opencode (`run --model`) and agy (`--model`); without it the agent's default applies, and a free-tier refusal is reported with a re-pair hint instead of a bare exit code.
