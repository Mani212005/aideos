...
## Jev shot-visual choice, SVG routing, and vision judge
- `backend/jev.ts` is the single shared decision client (CHOICE plus confidence gating plus heuristic fallback): primitive choice plus `selectShotVisual` (Text/StatCounter/TokenStrip/Plot/MatrixGrid/Distribution/LayerStack/ScaleBar), `selectSvgRoute` (standard-blocks vs svg-asset), and `judgeVisionStill` (rendered PNG base64 plus intent plus clip/asset ids to score plus reasons). Do not fork a second client; reuse its endpoint/timeout/mock shape.
- `backend/pipeline/design.ts`'s `compileFilmFromScreenplayAsync` is the compile path: model-authored visuals validated against `blockSchema`, SVG-routed beats recorded as `svgAssets` with `metaphor: "custom"`, footage precedence preserved. `backend/pipeline/run.ts` design stage uses it.
- `backend/scene/visionJudge.ts`'s `synthesizeSvgAssetWithVision` mirrors `generateSvg.ts` validate-repair: validate the asset, rasterize a still with headless Chrome via `renderStill.ts` (never qlmanage), judge look-alike score, retry with failures fed back. Inject rasterize/judge in tests; pass through when Chrome or the key is missing.
